function [env1_fts2,  env2_fts1] = make_band_chimeras(name1, name2, Nbands, do_play, do_plot,Fs,save_path)
...
if do_plot	% plot waveforms of original signals
	...
	subplot(2,1,1)
	plot(t, orig1);
	title(sprintf('Original "%s" Sound', '- stim 1'))
	subplot(2,1,2)
	plot(t, orig2)
	title(sprintf('Original "%s" Sound', '- stim 2'))
	xlabel('Time (ms)')
	drawnow
end
